...
### Вариант B — умный локальный preview
```bash
./scripts/local_preview.sh
```
Скрипт сначала пытается поднять полный Spring Boot UI, а если backend недоступен — автоматически переключается на статический preview `http://localhost:8081/preview.html`.

### Вариант D — публикация как Android (Google Play)
...
### Вариант C — только UI preview
```bash
./scripts/ui_preview.sh
```
Подходит, если хочешь быстро посмотреть интерфейс без backend.
...
### 2) Быстрый локальный сценарий (автоматический, с fallback на static preview)
```bash
./scripts/local_preview.sh
```
