...
public class MediaController {
    ...
    @GetMapping
    public List<Map<String, Object>> list(@RequestHeader("X-RentFlow-Token") String token) {
        authorize(token); return storage.list("demo-user").stream().map(a -> Map.<String,Object>of("id",a.getId(),"name",a.getOriginalName(),"kind",a.getKind(),"type",a.getMediaType(),"size",a.getOriginalSize(),"createdAt",a.getCreatedAt())).toList();
    }
    ...
    public ResponseEntity<InputStreamResource> download(@RequestHeader("X-RentFlow-Token") String token, @PathVariable UUID id) throws Exception {
        ...
        return ResponseEntity.ok().contentType(MediaType.parseMediaType(asset.getMediaType()))
